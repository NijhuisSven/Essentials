package nl.nijhuissven.orbit.database;
...
import nl.nijhuissven.orbit.config.DatabaseType;
import nl.nijhuissven.orbit.config.GlobalConfiguration;
import nl.nijhuissven.orbit.database.adapters.FixedBooleanAdapter;
...
public class Database {
    ...
    public Database(GlobalConfiguration config, File dataFolder) {
        ...
        try {
            ...
            Orbit.logger().severe("Database initialization failed: " + e.getMessage());
    ...
    public void saveModel(StormModel model) {
        try {
            ...
            Orbit.logger().severe("Couldn't save model: " + e.getMessage());
        }
    }
    ...
    public <T extends StormModel> List<T> query(Class<T> clazz, String field, Object value) {
        ...
        try {
            ...
            Orbit.logger().severe("Database query failed: " + e.getMessage());
        }
    ...
    public void deleteModel(StormModel model) {
        try {
            ...
            Orbit.logger().severe("Couldn't delete model: " + e.getMessage());
        }
    }
} 